...
fn plonk_prove_full_execution(vp: &String, address: &String) -> Result<(Vec<u8>, Vec<u8>), String> {
  let client = ProverClient::new();
  let mut stdin = SP1Stdin::new();
  stdin.write(vp);
  stdin.write(address);
  println!("Setting up proof...");
  let (pk, _) = client.setup(FULL_EXECUTION_ELF);
  println!("Proving...");
  ...
  let proof = full_execution_proof.unwrap();

  return Ok((proof.public_values.to_vec(), proof.bytes()));
}
